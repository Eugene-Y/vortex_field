...
export class PingPongFramebuffer {
  constructor(gl, width, height, internalFormat, format, type) {
    this._gl = gl;
    this._buffers = [
      createTextureAndFramebuffer(gl, width, height, internalFormat, format, type),
      createTextureAndFramebuffer(gl, width, height, internalFormat, format, type),
    ];
    this._readIndex = 0;
  }
...
export class SingleFramebuffer {
  constructor(gl, width, height, internalFormat, format, type) {
    ...
    const { texture, framebuffer } = createTextureAndFramebuffer(
      gl, width, height, internalFormat, format, type
    );
...
function createTextureAndFramebuffer(gl, width, height, internalFormat, format, type) {
